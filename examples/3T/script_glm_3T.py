...
def make_contrasts(sessions, n_reg=7):
    ...
    (positive wedge, negative wedge)
    """
    wedge_pos, wedge_neg = sessions
    con_ids = ['sin_wedge_pos', 'cos_wedge_pos', 'sin_wedge_neg',
               'cos_wedge_neg']
...
for subject in subjects:
    for side in sides:
        ...
        write_array[mask_array] = st.norm.isf(st.f.sf(F_ / 4, 4, 240))
        if side == False:
            contrast_image = Nifti1Image(write_array, affine)
            save(contrast_image, 
